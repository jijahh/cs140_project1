...
class MLFQ:
    ...
    def run(self):
        while True:
            ...
            for i in range(len(self.queue_list)):
                self.to_print[2][i].append(
                    [j.name for j in self.queue_list[i].ready_queue]
                )
            self.to_print[3].append(self.active_queue.ready_queue[0])
    ...
    def print_everything(self):
        ...
        print("Queues :", end=" ")
        print(to_print[2])
        print(f"CPU: {to_print[3]}")
        print(f"I/O: {to_print[4]}")
